...
def main():
    ...
    StepLR = torch.optim.lr_scheduler.StepLR(optimizer, step_size=17000, gamma=0.5)
    ...
    selected1= np.where(train_data_all[:,1]==0)
    train_data_boundary1 = train_data_all[selected1]
    selected2 = np.where(train_data_all[:, 1] == 2 * pi)
    train_data_boundary2 = train_data_all[selected2]
    selected3 = np.where(train_data_all[:, 2] == 0)
    train_data_boundary3 = train_data_all[selected3]
    selected4 = np.where(train_data_all[:, 2] == 2 * pi)
    train_data_boundary4 = train_data_all[selected4]
    train_data_boundary = np.vstack((train_data_boundary1, train_data_boundary2,train_data_boundary3,train_data_boundary4))

    train_data_boundary = torch.from_numpy(train_data_boundary).float()
    train_data_boundary = train_data_boundary.to(device)
    bo_data = train_data_boundary[:,:3]
    exact_p = train_data_boundary[:,5:6]
    ...
    print('p')
    error_p_relative = np.linalg.norm(train_data_all[:, 5:6] - p_pred, 2) / np.linalg.norm(train_data_all[:, 5:6], 2)
    error_p_abs = np.mean(np.abs(train_data_all[:, 5:6] - p_pred))
    print('L2:', error_p_relative)
    print('L1:', error_p_abs)

    scipy.io.savemat('taylorb1.mat' , {'u_pred':u_pred, 'v_pred':v_pred, 'p_pred':p_pred})
